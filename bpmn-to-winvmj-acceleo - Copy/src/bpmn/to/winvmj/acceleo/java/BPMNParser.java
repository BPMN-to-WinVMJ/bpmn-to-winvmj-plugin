...
public class BPMNParser {
    ...
    public static void addElements(BPMN bpmn, org.eclipse.bpmn2.Process process) throws Exception {
    ...
    private static Component findFlow(BPMN bpmn) {
        ...
        for (ParallelGateway ic : bpmn.getGf().values()) {
            ...
            for (SequenceFlow f : outFlows) {
            	...
            	System.out.println("[DEBUG]mid" + mid + " " + !(hasOneInOut(mid)) + " " + !(mid instanceof StartEvent) + " " + !(mid instanceof EndEvent));
                ...
                if (oc == null) {
                	System.out.println("[DEBUG] target " + target + " " + !(target instanceof ParallelGateway) + " " + (target instanceof ParallelGateway && ((ParallelGateway)target).getOutgoing().size() > 1 ));
            ...
            System.out.println("[DEBUG] oc " + bpmn.getGj());
