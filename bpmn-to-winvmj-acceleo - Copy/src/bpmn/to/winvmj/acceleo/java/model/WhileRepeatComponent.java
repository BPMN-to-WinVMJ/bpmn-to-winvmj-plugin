...
public class WhileRepeatComponent extends Component implements Looping {
	...
	public String getFromStartToUser(String bpmnName, Map<String, String> usedVariables, int indent) {
		...
		if (topContinuable) {
	        ...
	        GenerateQuery.buildResource(builder, bpmnName, this.getStart().getOutgoing().get(0).getTargetRef(), new HashSet<>(visited), usedVariables, indent);
	        ...
	        GenerateQuery.buildResource(builder, bpmnName, this.getStart().getOutgoing().get(0).getTargetRef(), new HashSet<>(visited), usedVariables, indent + 1);
			...
			GenerateQuery.buildResource(builder, bpmnName, this.getStart().getOutgoing().get(0).getTargetRef(), visited, usedVariables, indent + 1);
			return builder.toString();
		}
		return "";
	}
